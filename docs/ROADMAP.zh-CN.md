...
- [ ] 修复 `_score_bgm_candidate` 多功能性奖励 bug（分支条件 `emo == mood` 恒为 False）
- [ ] 多语言反 AI 腔 — 扩展禁用词列表至中文以外（英文、日文、韩文）
- [ ] Judge 五维度评审 — 新增反 AI 腔遵守度、叙事原则符合度
- [ ] Beat 去重 — Phase 2 扩展前检测并合并重复剧情节点
- [ ] 内置 Hook 模板库 — 用户未提供 `hook_templates` 时的后备方案
- [ ] 脚本级 QA 门 — TTS 前校验脚本（长度、多样性、hook 存在性）

### v0.5.9 — 语音与音频质量（规划中）

- [ ] TTS 时长反馈 v2 — 溢出段按速度调整重合成（替代 v1 仅调 pause 方案）
- [ ] TTS 输出质量验证 — 削波检测、SNR 估算、静音检查
- [ ] 情感感知 TTS — 基于 beat 情感标签的 SSML prosody 标记
- [ ] BGM 动态过渡 — 情绪区间变化时曲目淡入淡出
- [ ] 音频质量聚合 — 每段音频指标写入 metadata.json

### v0.5.10 — 字幕与翻译质量（规划中）

- [ ] 字幕 QA — CPS（字符/秒）、重叠检测、行长度合规
- [ ] 翻译术语一致性 — 跨分块术语表提取与强制统一
- [ ] 字幕显示长度校验 — 确保翻译后文本适配渲染区域
- [ ] 双语字幕优化 — 双语模式行平衡
- [ ] 标记未翻译行 — 输出元数据中标注回退原文的行
