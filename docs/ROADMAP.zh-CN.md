...
| v0.9.x | 可靠性与批量 | 熔断器、重试策略框架、任务检查点、优雅关闭、批量提交+聚合进度、cron 定时调度、死信队列、条件性分布式渲染。`CONTRACT_VERSION` → `(0, 9, 4)` |
...
- [x] OpenAPI 规范 — 自动生成的 API 文档（v0.8.2）

### v0.9.x — 可靠性与批量（已交付）

> **目标**：长时间运行不丢任务，批量出片有调度。
> 于 v0.9.1–v0.9.4 分阶段交付（2026-08-03）。`CONTRACT_VERSION` → `(0, 9, 4)`。

- [x] 熔断器 — 针对外部 API（LLM、TTS、TMDB、VLM）（v0.9.1）
- [x] 任务检查点 — 为长时间运行的渲染保存中间状态，支持断点续跑（v0.9.2）
- [x] 优雅关闭 — 退出前排空在途任务（v0.9.2）
- [x] 重试策略框架 — 可配置的逐步骤重试策略（任务级指数退避已在 `cloud/worker.py` 实现）（v0.9.1）
- [x] 批量任务提交 — 一次 API 请求提交 N 部影片（v0.9.3）
- [x] 定时任务 — 基于 cron 的周期性任务提交（v0.9.3）
- [x] 批量进度跟踪 — 跨子任务的聚合进度（v0.9.3）
- [x] 死信队列 — 失败任务转入 DLQ 供检查和重放（v0.9.4）
- [x] 分布式渲染（条件特性） — 触发条件：单机渲染 > 10 分钟且有多节点；基于 v0.8.0 容器化构建（v0.9.4）
- [x] CONTRACT_VERSION → `(0, 9, 4)` — 可靠性与批量类型通过 SDK 导出
