...
> **Definition of Done for the v1.4.2 release.** Every item on this list
> must be verified and checked off before the v1.4.2 tag is created and the
...
- [x] **Test coverage meets threshold**
  ...
  - Note: Threshold defined in CI config (`.coveragerc` + `ci.yml`); v1.4.2 measured 91.25%; must not regress from the v1.1 baseline
...
- [ ] **New v1.4.2 ecosystem tests pass**
  - Command: `pytest -v tests/test_v142_premiere.py tests/test_v142_cli.py`
  - Expected: All pass (Premiere XML structure, whitelist, CLI wrappers, dry-run plan)
  - Note: Adds +26 tests total vs v1.4.1 (23 unit + 3 integration-marked)
...
- [x] **CHANGELOG.md is finalized**
  ...
  - Expected:
    - New `## [1.4.2] - <date>` heading (was `[Unreleased]`)
    - `CONTRACT_VERSION` line uses the mandated format: `- `CONTRACT_VERSION` remains (1, 3, 0). All NNN tests pass (N skipped in CI, 0 failures). +M new tests vs v1.4.1.`
    - Version comparison links at bottom updated (`[Unreleased]` → `.../compare/v1.4.2...HEAD`, new `[1.4.2]` link)
    - Historical entries unchanged (no codename or terminology edits to old releases)

- [x] **ROADMAP reflects v1.4.2**
  - Verification: `docs/ROADMAP.md` (+ `.zh-CN.md`)
  - Expected: v1.4.2 row in the Completed table; the v1.4 section marks all three releases shipped

- [x] **Current-version alignment**
  - Method: Scan **all** public docs (`docs/**/*.md` + `README.md`) — do **not** rely on a fixed file list — and grep each for the previous version string (`v1.4.1`) used as a "current"-version claim
  - Expected: No public doc still claims the old version as current; update every stale stamp (compatibility notes in `DEPLOYMENT`/`MIGRATION`/`TUTORIAL`, `mn version` output in `QUICKSTART`, `index.md` checklist label) to **v1.4.2**. Leave legitimate references intact (historical records, comparison baselines, illustrative examples). Re-run the scan to confirm.
...
- [ ] **Version numbers are aligned**
  - Verification:
    ...
    - `src/movie_narrator/contract.py` → `CONTRACT_VERSION = (1, 3, 0)` (unchanged — no new exports in v1.4.2, do **not** bump)
    - `docs/ROADMAP.md` → CONTRACT_VERSION line shows `(1, 3, 0)` (unchanged in v1.4.2)
    - `docs/MIGRATION.md` → current-version note updated
  - Expected: Package version 1.4.2; contract version remains (1, 3, 0)

- [ ] **Tag naming follows convention**
  - Format: `v1.4.2` (lowercase `v`, semver, no prefix/suffix)
  - Command: `git tag -a v1.4.2 -m "v1.4.2 - Ecosystem: Premiere Timeline Adapter & CLI Ergonomics"`
...
- [ ] **PyPI publish workflow is ready**
  ...
  - Manual verification:
    ...
    mn version  # should show 1.4.2
    ```

- [ ] **GitHub Release follows the release.md spec**
  - Title: `v1.4.2 - Ecosystem: Premiere Timeline Adapter & CLI Ergonomics`
  - Body: the `## [v1.4.2]` section of `CHANGELOG.md` copied verbatim (per `.claude/rules/release.md`), plus a link to the full CHANGELOG
  - Exactly one **non-draft** release per tag — delete any empty draft left by `publish.yml`

- [ ] **Git tag pushed**
  - Command: `git push origin v1.4.2`
  - Expected: Tag appears on GitHub, publish workflow starts, PyPI `movie-narrator==1.4.2` published
...
- [ ] **PyPI release verified**
  - Verification:
    ...
    # Expected: (1, 3, 0)
    ```
  - Expected: Package installs cleanly, import works, package version 1.4.2

- [ ] **Maintenance branch exists**
  - Verification: `v1.4.x` branch present on origin (created at v1.4.2)
