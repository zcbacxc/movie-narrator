...
v1.0.0 是 movie-narrator 的首个**稳定版**。公共 API——由 `CONTRACT_VERSION (1, 0, 0)` 定义——现已冻结，并将在整个 v1.x 系列中保持向后兼容。

### v1.0 新增内容

- **API 冻结**：所有从 `movie_narrator.contract` 导出的符号均声明为稳定。v1.x 中不会发生破坏性变更。
- **正式稳定性承诺**：完整的版本化、弃用和升级保障政策请参见 [STABILITY.zh-CN.md](STABILITY.zh-CN.md)。
- **发布清单**：[RELEASE_CHECKLIST.zh-CN.md](RELEASE_CHECKLIST.zh-CN.md) 定义了 v1.0 的完成定义——发布前已逐项验证。
- **保留所有 v0.9.x 功能**：v1.0.0 未移除任何功能或 API。MAJOR 版本提升标志着稳定性的开始，而非破坏性重写。

### CONTRACT_VERSION 变更

| 之前（v0.9.7） | 之后（v1.0.0） | 含义 |
|---------------|----------------|------|
| `(0, 9, 5)` | `(1, 0, 0)` | MAJOR 提升——首个稳定版，API 冻结 |

这是 semver 意义上的 MAJOR 版本提升，但**并不意味着** API 被移除或更改。实际上：

- v0.x 是快速开发的预稳定系列，次版本之间可能发生破坏性变更。
- v1.0 标志着 API 声明稳定的时刻。v0.9.6 中存在的同一 API 表面现已冻结，并在整个 v1.x 系列中得到保障。
- 基于 v0.9.x 构建的插件和外部消费者将无需改动即可在 v1.0.0 中继续工作。

### 升级步骤（摘要）

1. **升级软件包**：`pip install --upgrade movie-narrator`
2. **在作业文件中将 `format` 更新为 `video_format`**（如果仍在使用已弃用的键）
3. **验证配置边界**——`.env` 中不应有流水线参数
4. **运行 `mn version`** 确认 v1.0.0
5. **向你的插件添加 `check_version((1, 0, 0))`** 以声明稳定 API 依赖

详细说明请参见下文[分步升级流程](#分步升级流程)。
...
**当前版本（v1.7.0）：** `CONTRACT_VERSION` 为 `(1, 4, 0)`（M4 声明式 StepSpec 契约字段）。
