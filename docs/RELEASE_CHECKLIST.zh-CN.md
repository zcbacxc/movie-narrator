...
> **v1.3.0 版本的完成定义（Definition of Done）。** 在创建 v1.3.0 标签
...
- [x] **测试覆盖率达标**
  ...
  - 说明：阈值在 CI 配置中定义（`.coveragerc` + `ci.yml`）；v1.3.0 实测 90.94%；不得低于 v1.1 基线
...
- [ ] **新增 v1.3.0 工作流语义测试通过**
  - 命令：`pytest -v tests/test_v130_rerun.py tests/test_v130_dag.py tests/test_v130_deliverable.py`
  - 预期结果：全部通过（选择性重跑、步骤图契约、交付清单）
  - 说明：相对 v1.2.1 总计新增 +54 个测试
...
- [x] **CHANGELOG.md 已定稿**
  - 验证：审阅 `CHANGELOG.md`
  - 预期结果：
    - 新增 `## [1.3.0] - <日期>` 标题（原为 `[Unreleased]`）
    - `CONTRACT_VERSION` 行使用规范格式：`- \`CONTRACT_VERSION\` bumped to (1, 1, 0). All NNN tests pass (N skipped in CI, 0 failures). +M new tests vs v1.2.1.`
    - 底部版本比较链接已更新（`[Unreleased]` → `.../compare/v1.3.0...HEAD`，新增 `[1.3.0]` 链接）
    - 历史条目保持不变（不对旧版做代号或措辞改动）

- [x] **ROADMAP 反映 v1.3.0**
  - 验证：`docs/ROADMAP.zh-CN.md`（及 `.md`）
  - 预期结果：已完成表中含 v1.3.0 行；v1.3 范围的前三项标注已随 v1.3.0 交付

- [x] **当前版本对齐**
  - 方法：扫描**所有**公开文档（`docs/**/*.md` + `README.md`）——**不要依赖固定文件清单**——逐个 grep 上个版本号（`v1.2.1`）作为"当前"版本声明的使用
  - 预期结果：无任何公开文档仍将旧版本当作当前版本；将每一处过期戳（`DEPLOYMENT`/`MIGRATION`/`TUTORIAL` 中的兼容性说明、`QUICKSTART` 中 `mn version` 输出、`index.md` 发布清单标签）更新为 **v1.3.0**。合理的历史引用保持不动（历史记录、比较基线、示例性表述）。重跑扫描确认。
...
- [ ] **版本号已对齐**
  - 验证：
    ...
    - `src/movie_narrator/contract.py` → `CONTRACT_VERSION = (1, 1, 0)`（已提升——v1.3.0 工作流语义导出）
    - `docs/ROADMAP.zh-CN.md` → CONTRACT_VERSION 行显示 `(1, 1, 0)`（v1.3.0 已提升）
    - `docs/MIGRATION.zh-CN.md` → 当前版本注记已更新
  - 预期结果：包版本 1.3.0；契约版本提升至 (1, 1, 0)

- [ ] **标签命名遵循约定**
  - 格式：`v1.3.0`（小写 `v`、语义化版本、无前缀/后缀）
  - 命令：`git tag -a v1.3.0 -m "v1.3.0 - Workflow Semantics: Selective Rerun, DAG Contract & Deliverable Manifest"`
...
- [ ] **PyPI 发布工作流就绪**
  ...
  - 手动验证：
    ...
    mn version  # 应显示 1.3.0
    ```

- [ ] **GitHub Release 遵循 release.md 规范**
  - 标题：`v1.3.0 - Workflow Semantics: Selective Rerun, DAG Contract & Deliverable Manifest`
  - 正文：逐字复制 `CHANGELOG.md` 的 `## [v1.3.0]` 章节（按 `.claude/rules/release.md`），并附完整 CHANGELOG 链接
  - 每个标签只允许一个**非草稿** Release —— 删除 `publish.yml` 可能遗留的空草稿

- [ ] **Git 标签已推送**
  - 命令：`git push origin v1.3.0`
  - 预期结果：标签出现在 GitHub 上，发布工作流启动，PyPI 发布 `movie-narrator==1.3.0`
  - 说明：仅在所有清单项确认后推送标签
...
- [ ] **PyPI 发布已验证**
  - 验证：
    ...
    # 预期结果：(1, 1, 0)
    ```
  - 预期结果：包干净地安装，导入正常，包版本 1.3.0

- [ ] **维护分支存在**
  - 验证：origin 上存在 `v1.3.x` 分支（v1.3.0 时创建）
