...
- **Settings ops view** — `config.ServerOpsSettings` (frozen, read-only) plus `get_server_ops()` exposes the deployment-facing knobs (`api_key` / webhook / rate-limit / scheduler / circuit-breaker / distributed / graceful-shutdown) to the `cloud/` subsystem, decoupling them from the LLM/TTS/TMDB settings surface. Existing `MN_*` environment variables are unchanged (zero breakage); a disjointness test prevents future ops-fields from being added to `Settings` without being routed into a view. `docs/ARCHITECTURE.md` and `.env.example` are updated to state the layered boundary honestly.
- **Metadata key integrity gate** — `scripts/check_metadata_keys.py` (wired into CI lint) fails the build if any string-literal `metadata[...]` / `.get(...)` key in `src/` is not declared in `models.MetadataDict`; the 10 previously-undeclared keys (`reference_media`, `reference_media_captions`, `prompt_cache`, `plan`, `plan_policy`, `artifact_retention`, `rerun`, `usage`, `match_visual_features`, `render_main_encode_timeout`) are now declared.
- **Mega-file size gate** — `scripts/lint_file_size.py` fails on newly-added source files over 900 lines while treating pre-existing over-budget files as non-fatal, so the debt is tracked without blocking the build.
...
- **Principal & tenant foundation** — tasks, artifacts, and audit records now carry `tenant_id` / `principal`: API-key callers get a configurable principal (`MN_API_PRINCIPAL`), optional `X-MN-Tenant` scoping, and per-tenant artifact listing (non-default tenants see only their own tasks; the `default` tenant keeps the full view). SQLite storage migrates additively; local loopback behaviour unchanged.
- **Plans & entitlements** — `cloud/entitlements.py` models plans (max duration / resolution / artifact bytes, watermark policy, GPU-encoder permission, artifact TTL) with `default` (unlimited — unchanged behaviour), `free` (720p/60s/watermark/CPU/24h) and `pro` (1080p/3600s/8 GiB/GPU/72h) built-ins plus JSON overrides (`MN_PLANS_FILE`, `MN_DEFAULT_PLAN`). Enforced at API submission (403 `entitlement_denied`) and via worker-side injection (mandatory watermark, forced CPU encoder) — pipeline code untouched. (ADR-013)
- **Webhook MVP** — task terminal states (completed / failed / cancelled) emit signed events: HMAC-SHA256 `X-MN-Signature`, idempotent `X-MN-Event-Id`, retry with backoff via the shared retry framework, JSONL delivery records; configured via `MN_WEBHOOK_URLS` / `MN_WEBHOOK_SECRET` / `MN_WEBHOOK_TIMEOUT` / `MN_WEBHOOK_MAX_RETRIES`, disabled when unset, and never affects task outcome. (ADR-013)
- **Dashboard contract** — stable, versioned `GET /api/v1/dashboard/summary` (task counts by status, recent tasks, queue depth, artifact stats, configured plans) for the external `movie-narrator-web` UI.
- **Tests** (`tests/test_v131_tenant_principal.py`, `tests/test_v131_entitlements.py`, `tests/test_v131_webhooks.py`, `tests/test_v131_dashboard.py`, `tests/test_contract.py`): +95 tests covering tenant scoping, plan enforcement, webhook signing/retry/isolation, and the dashboard schema.

### Changed
- `CONTRACT_VERSION` bumped to (1, 2, 0): new contract exports (`Plan`, `EntitlementError`, `WebhookEvent`, `WebhookDispatcher`, `build_dashboard_summary`). All 2713 tests pass (2 skipped in CI, 0 failures). +95 new tests vs v1.3.0.
...
- **Persistent GPU capability cache** — `detect_gpu_encoder` now persists successful probe results on disk (keyed to the resolved ffmpeg binary, platform, and CI flag), so fresh processes and worker restarts reuse a previous detection instead of re-running `ffmpeg -encoders`. Clean negative (no-GPU) results are cached too; probe failures never are; writes are atomic and any cache failure silently falls back to a live probe.
- **Encoder fallback-reason reporting** — `encoder_info` in `metadata.json` now includes a `fallback_reason` field so it is visible at a glance why a GPU encoder was not used (no ffmpeg, CI skip, probe failure, no GPU detected, unknown hint, or an in-render GPU→CPU downgrade).
- **Runtime GPU→CPU fallback auditable** — when a hardware encode throws mid-render and the pipeline retries with `libx264`, `metadata.json` reports the encoder actually used plus the reason, and a structured `encoder_fallback` log record is emitted for observability.
- **Tests** (`tests/test_v121_gpu_cache.py` + `tests/test_render.py`): +18 tests covering the capability-cache hit/miss/invalidation paths and encoder fallback-reason reporting.

### Changed
- `CONTRACT_VERSION` remains (1, 0, 0). All 2565 tests pass (1 skipped in CI, 0 failures). +18 new tests vs v1.2.0.
