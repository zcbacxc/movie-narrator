...
and registers it via :func:`~movie_narrator.plugins.contracts.load_plugin`.

Error handling is per-plugin: a broken plugin produces a warning but
does not prevent other plugins from loading.

Moved here from ``movie_narrator.plugin_loader`` (M1). Imports
``Plugin`` / ``load_plugin`` from :mod:`.contracts` — never from
``contract`` or ``plugin_loader``.
"""
...
from .contracts import Plugin, load_plugin
...
def _load_entry_point(ep: Any) -> Plugin:
    ...
    :class:`~movie_narrator.plugins.contracts.Plugin` protocol — i.e. it has
...
def discover_plugins(*, group: str = ENTRY_POINT_GROUP) -> List[PluginLoadResult]:
    ...
    :func:`~movie_narrator.plugins.contracts.load_plugin`.
